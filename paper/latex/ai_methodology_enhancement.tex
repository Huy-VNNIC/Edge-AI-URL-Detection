...
\noindent \textbf{Note:} Model prediction time measured for single-sample inference. Throughput measured as end-to-end samples per second including feature extraction overhead under single-threaded conditions.
...
\begin{enumerate}
\item \textbf{Realistic Performance Benchmarking}: Our evaluation demonstrates achievable performance levels for edge-AI URL detection, with Random Forest achieving 72.30\% accuracy (F1-score: 0.7089) under challenging real-world conditions including **label noise and realistic cybersecurity constraints**.
...
\begin{itemize}
\item \textbf{Algorithm Robustness}: Testing across linear (LR), kernel-based (SVM), ensemble (RF, XGBoost), and neural approaches under challenging conditions including **label noise and edge deployment constraints**
...
\begin{enumerate}
\item \textbf{Superior Performance}: Achieves the **highest F1-score (0.7089)** with **comparable accuracy (72.30%)**, while Logistic Regression attains slightly higher accuracy (72.37%) but lower F1-score, under realistic cybersecurity conditions with **label noise and deployment constraints**
...
The key AI innovations include: (1) **Feature engineering framework** that enables multiple AI paradigms to reach **realistic cybersecurity performance levels (70-72% accuracy range)**, (2) **Systematic AI model comparison** providing **empirical** selection guidelines for edge deployment, (3) **Quantified performance trade-off analysis** revealing **112-408 samples/sec throughput variations** critical for deployment decisions, and (4) **Resource-aware AI optimization** demonstrating how classical models can outperform complex approaches in constrained environments.
