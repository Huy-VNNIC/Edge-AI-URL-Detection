...
\subsection{Hardware Compatibility \& Integration}

{\small
\textbf{Tested Platforms:} Raspberry Pi 4 (10.5ms, optimal), Jetson Nano (8.2ms, optimal), Industrial PLC (18.7ms, use 23-feature mode), BeagleBone (31.4ms, 19-feature minimal mode). ESP32 not supported (insufficient memory).

\textbf{SIEM Integration:} Forward JSON responses to Splunk/ELK/Sentinel for correlation. \textbf{Firewall Modes:} Inline blocking (score$>$0.70), passive monitoring, or hybrid (auto-block$>$0.85, alert 0.60-0.85). \textbf{Threat Intel:} Export to MISP/ThreatConnect, subscribe to URLHaus/PhishTank via STIX/TAXII.
}
...
\begin{table}[htbp]
\centering
\caption{Annual TCO: Edge-AI vs. Cloud}
{\scriptsize
\begin{tabular}{lp{1.5cm}p{1.5cm}}
\toprule
Cost Factor & Edge-AI & Cloud API \\
\midrule
Infrastructure & \$50-200 & \$0 \\
API/Service & \$0 & \$5-25K \\
Bandwidth & \$0 & \$500-2K \\
Maintenance & \$500-1.5K & \$0 \\
Compliance & \$0 & \$1-5K \\
\midrule
\textbf{Year 1} & \textbf{\$550-1.7K} & \textbf{\$6.5-32K} \\
\bottomrule
\end{tabular}
}
\end{table}
...
Break-even: 3-6 months for $>$50K daily queries; \$50-100K annual savings at $>$500K queries.

\textbf{Key Operator Concerns:} \textit{Model updates?} Blue-green deployment with 24hr rollback. \textit{Training poisoning?} Source from verified feeds (URLHaus), statistical outlier detection. \textit{72\% accuracy sufficient?} Deploy hybrid: edge blocks obvious threats (10ms), escalate uncertain to cloud/human (25-30\% cases) for 80-85\% effective accuracy. \textit{Management justification?} \$6-30K savings, GDPR/HIPAA compliance, 24/7 resilience during cloud outages.
